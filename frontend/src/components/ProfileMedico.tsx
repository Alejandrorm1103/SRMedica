...
import { User, Mail, Phone, MapPin, Building, GraduationCap, Save, ArrowLeft, Calendar, FileText, CheckCircle2 } from "lucide-react";
import { useAuth } from "../context/AuthContext";
import { toast } from "sonner";
import { TIPOS_DOCUMENTO, SEXOS } from "../constants/catalogos";
import api from "../services/api";

export function ProfileMedico() {
    const { user } = useAuth();
            ...
            try {
                ...
                if (data) {
                    ...
                    const normalizeCatalogValue = (
                        ...
                        // Si viene el ID directo (ej: 1, "1")
                        return String(val);
                    };

                    setFormData(prev => ({
                        ...prev,
                        primerNombre: d?.primer_nombre || prev.primerNombre,
                        segundoNombre: d?.segundo_nombre || "",
                        primerApellido: d?.primer_apellido || prev.primerApellido,
                        segundoApellido: d?.segundo_apellido || "",
                        tipoDocumento: normalizeCatalogValue(d?.tipo_documento, TIPOS_DOCUMENTO),
                        numeroDocumento: d?.numero_documento || "",
                        sexo: normalizeCatalogValue(d?.sexo, SEXOS),
                        fechaNacimiento: d?.fecha_nacimiento ? d.fecha_nacimiento.split('T')[0] : "",
                        telefono: d?.telefono || "",
                        direccionLinea1: d?.direccion || "",
                        ciudad: d?.ciudad || "",
                        departamento: d?.departamento || "",
                        pais: d?.pais || "",
                        cedula: d?.registro_profesional || "",
                        especialidad: d?.especialidades?.[0]?.nombre || d?.especialidad || "",
                        hospital: d?.hospital || "",
                    }));
                }
            } catch (error) {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            await api.patch('/doctors/me', {
                ...
                pais: formData.pais,
                registroProfesional: formData.cedula,
            });
    ...
    const displayName = formData.primerNombre && formData.primerApellido
        ? `Dr. ${formData.primerNombre} ${formData.primerApellido}`
        : "Médico";
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-[#024b85] tracking-tight">Mi Perfil Profesional</h1>
                            <p className="text-blue-600/80 text-sm">Gestiona tu información personal y profesional</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="md:col-span-1 space-y-6">
                        <Card className="bg-white rounded-[2rem] p-6 shadow-xl shadow-blue-900/5 border border-blue-100 overflow-hidden relative">
                            ...
                            <div className="flex flex-col items-center text-center relative z-10 pt-4">
                                ...
                                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100/50 text-blue-700 border border-blue-200/50 mb-6">
                                    {formData.especialidad || "Médico Especialista"}
                                </span>
                                ...
                                <div className="w-full space-y-4 border-t border-blue-100 pt-6">
                                    {formData.hospital && (
                                        <div className="flex items-center gap-3 text-sm text-blue-700 bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                                            ...
                                            <span className="truncate font-medium">{formData.hospital}</span>
                                        </div>
                                    )}
                                    <div className="flex items-center gap-3 text-sm text-blue-700 bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                                        <div className="p-2 bg-white rounded-lg shadow-sm">
                        ...
                        <Card className="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[2rem] p-6 shadow-lg border-none relative overflow-hidden">
                            ...
                            <div className="space-y-3 relative z-10">
                                ...
                                <div className="flex items-center gap-3 text-sm text-slate-300">
                                    <div className="h-2 w-2 rounded-full bg-emerald-400"></div>
                                    <span>Videoconsulta Habilitada</span>
                                </div>
                            </div>
                        </Card>
                    </div>
                        ...
                        <Card className="bg-white rounded-[2rem] shadow-xl shadow-blue-900/5 border border-blue-100 overflow-hidden">
                            <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-5">
                                            <div className="space-y-2">
                                                ...
                                                <Input id="direccionLinea1" name="direccionLinea1" value={formData.direccionLinea1} onChange={handleChange} style={{ backgroundColor: '#FFFFFF' }} className="!h-12 !rounded-xl !border-blue-200 focus:!border-blue-400 transition-all text-blue-900" placeholder="Dirección consultorio/residencial..." />
                                            </div>
                                            <div className="grid md:grid-cols-2 gap-5">
                                                <div className="space-y-2">
                                ...
                                <div className="p-8 border-b border-blue-100">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-5">
                                        <div className="space-y-2">
                                            <Label htmlFor="especialidad" className="text-blue-900 font-medium">Especialidad</Label>
                                            <Input id="especialidad" name="especialidad" value={formData.especialidad} onChange={handleChange} style={{ backgroundColor: '#FFFFFF' }} className="!h-12 !rounded-xl !border-blue-200 focus:!border-blue-400 transition-all font-medium text-blue-900" placeholder="Ej: Cardiología" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label htmlFor="cedula" className="text-blue-900 font-medium">Cédula Profesional</Label>
                                            <Input id="cedula" name="cedula" value={formData.cedula} onChange={handleChange} style={{ backgroundColor: '#FFFFFF' }} className="!h-12 !rounded-xl !border-blue-200 focus:!border-blue-400 transition-all font-medium text-blue-900" />
                                        </div>
                                        <div className="space-y-2 md:col-span-2">
                                            <Label htmlFor="hospital" className="text-blue-900 font-medium">Hospital / Clínica</Label>
                                            <div className="relative">
                                                <Building className="absolute left-3 top-3.5 h-4 w-4 text-blue-400" />
                                                <Input id="hospital" name="hospital" value={formData.hospital} onChange={handleChange} style={{ backgroundColor: '#FFFFFF' }} className="!h-12 !pl-10 !rounded-xl !border-blue-200 focus:!border-blue-400 transition-all font-medium text-blue-900" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-8 bg-blue-50/30 flex justify-end gap-4 rounded-b-[2rem]">
                                    <Button type="button" variant="outline" onClick={() => navigate("/medico/home")} className="h-12 px-6 rounded-xl border-blue-200 text-blue-600 hover:bg-white hover:text-blue-800 hover:border-blue-300">
                                        Cancelar
                                    </Button>
